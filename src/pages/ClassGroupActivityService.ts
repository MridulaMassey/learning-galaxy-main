...
export interface ClassGroupActivity {
  ...
  // Service class for handling ClassGroupActivity data
  export const classGroupActivityService = {
    // Method to get all class group activities
    getClassGroupActivities: async (): Promise<ClassGroupActivity[]> => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch class group activities");
        }
        
        const data = await response.json();
        return data;
      } catch (error) {
        console.error("Error fetching class group activities:", error);
        // If the API call fails, fallback to mock data for demo purposes
        return mockClassGroupActivities;
      }
    }
  };
// Mock data for class group activities
const mockClassGroupActivities: ClassGroupActivity[] = [
  {
    id: 1,
    activityId: "act-101",
